...
class TestNoFailure(unittest.TestCase):
    ...
    def test_things_that_need_simulate(self):
        covers(['simulate','getReactionRatesEx','getRatesOfChangeEx',
                'getResultColumnLabel','getMatrixNumRows'])

        rrPython.simulate()
        rrPython.getReactionRatesEx([1.0,.9,.8,.6])
        rrPython.getRatesOfChangeEx([1.0,.9,.8,.6])
        rrPython.getResultColumnLabel()
        rrPython.getMatrixNumRows()
...
test('getReactionRate',0)
test('getRateOfChange',0)
test('getValue','S1')
test('listToString',['a','b'])
test('createVector',10)
test('setLogLevel','DEBUG4')
test('setTimeCourseSelectionList','time,S1,S')
test('getCompartmentByIndex',0)
test('getResultElement')
test('setGlobalParameterByIndex',0,1.0)
test('getFloatingSpeciesByIndex',0)
